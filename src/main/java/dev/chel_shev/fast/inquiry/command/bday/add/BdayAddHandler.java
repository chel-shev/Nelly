...
public class BdayAddHandler extends FastCommandInquiryHandler<BdayAddInquiry> {
    ...
    public void executionLogic(BdayAddInquiry i) {
        if (eventService.isExist(i.getName(), i.getUser(), i.getBdayDate())) {
            ...
            if (i.getBdayDate().withYear(now.getYear()).isAfter(now))
                bdayEvent = new FastBdayEventEntity(i.getName(), i.getBdayDate(), i.getCommand(), i.getBdayDate().withYear(now.getYear()).withHour(8).withMinute(0).withSecond(0), subscriptionService.getSubscription(i.getUser(), commandService.getCommand("/bday")));
            else
                bdayEvent = new FastBdayEventEntity(i.getName(), i.getBdayDate(), i.getCommand(), i.getBdayDate().withYear(now.getYear() + 1).withHour(8).withMinute(0).withSecond(0),  subscriptionService.getSubscription(i.getUser(), commandService.getCommand("/bday")));
    ...
    public BdayAddInquiry cancel(BdayAddInquiry i) {
        super.cancel(i);
        i.setKeyboardType(FastKeyboardType.REPLY);
        i.setKeyboardButtonList(asList("", ""));
        return i;
    }
    ...
    public void preparationLogic(BdayAddInquiry i, Message message) {
        if (fastUtils.getArgs(message.getText()).isEmpty()) {
            ...
            i.setKeyboardType(FastKeyboardType.REPLY);
            i.setKeyboardButtonList(keyboardService.getButton(CancelKeyboardInquiry.class));
        } else {
